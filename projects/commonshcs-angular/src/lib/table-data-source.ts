...
import { DocsDelegate, DocsService } from './docs';
...
export class DocsDelegateTableDataSource<T extends TableData> extends DataSource<T> {
...
export class DocsTableDataSource extends DataSource<TableData> {
  paginator: MatPaginator | undefined
  sort: MatSort | undefined
  lastRow: {[key: number]: TableData} = {}
  pageSize?: number
  ...
  connect(): Observable<TableData[]> {
    if (this.paginator && this.sort) {
      ...
      return combineLatest([
        ...
        switchMap(([page, sort, path, where]) => {
          const orderBy: {orderBy?: DocsDelegateQuery['orderBy']} = {}
          if (sort.active) {
            orderBy!.orderBy = [[sort.active, sort.direction]]
          }
